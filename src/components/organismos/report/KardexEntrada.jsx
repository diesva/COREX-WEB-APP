...
function KardexEntrada() {
  ...
  const { data: dataKardex = [] } = useQuery({
    queryKey: ["reporte kardex entrada", productoItemSelect.map(p => p.id)],
    queryFn: async () => {
  ...
  const productosConUltimaEntrada = dataKardex
    .map((movimientos, index) => {
      const ultimaEntrada = movimientos
        .filter((m) => m.tipo === "entrada")
        .sort((a, b) => new Date(b.fecha) - new Date(a.fecha))[0];
      return ultimaEntrada ? { ...ultimaEntrada, id: `${index}-${ultimaEntrada.id}`, producto: productoItemSelect[index] } : null;
    })
    .filter(Boolean);

  const productosMostrados = productosConUltimaEntrada.map(item => item.producto);
    ...
    ? dataKardex
        .map((movimientos, index) => {
          const ultimaEntrada = movimientos
            .filter((m) => m.tipo === "entrada")
            .sort((a, b) => new Date(b.fecha) - new Date(a.fecha))[0];
          return ultimaEntrada ? parseFloat(ultimaEntrada.cantidad || 0) * parseFloat(ultimaEntrada.preciocompra || 0) : 0;
        })
  ...
  dataKardex.forEach(movimientos => {
    ...
    if (ultimaEntrada) {
      const cuenta = ultimaEntrada.codigobarras || "SIN CUENTA";
      const cantidad = parseFloat(ultimaEntrada.cantidad) || 0;
      const precio = parseFloat(ultimaEntrada.preciocompra) || 0;
  ...
  const cantidadProductos = productosConUltimaEntrada.length;
    ...
    if (isHeader) {
      return (
        <View style={styles.row} key="header">
          ...
          <Text style={styles.cantDespHeaderCell}>CANT. INGRESADA</Text>
  ...
  const isGenerateDisabled = !dependencia || productosConUltimaEntrada.length === 0;
              ...
              <Page size="A4" orientation="landscape" style={styles.page}>
                ...
                <View style={styles.body}>
                  ...
                  <View style={styles.table}>
                    {renderTableRow({}, true)}
                    {productosConUltimaEntrada.length > 0 &&
                      productosConUltimaEntrada.map((ultimaEntrada, i) => renderTableRow(ultimaEntrada, false, i))}
                    {productosConUltimaEntrada.length > 0 && renderTableRow({}, false, null, true)}
                  </View>
